...
if (knitr::is_html_output()) {
  ...
  # For PDF output, use a static image
  NULL  # or .jpg
}
```
...
```{r plot, fig.width = 10, fig.fullwidth = TRUE}
staff_long %>%
  ggplot(aes(x = year, y = value, group = faculty_type, color = faculty_type)) +
  geom_line() +
  labs(
    title = "Number of Staff Examined Throughout the Years",
    subtitle = "Measured by Faculty Type",
    x = "Year",
    y = "Frequency",
    color = "Faculty Type"
  )
...
`I would mutate the data set to have a data set for the proportion of each faculty type.`

3.  Implement the changes you proposed in the previous exercise.

```{r mutate, eval = TRUE}
proportion_staff <- staff_long %>%
  group_by(value, faculty_type, year) %>%
  summarize(value = n()) %>%
  mutate(proportion = value / sum(value))
```

```{r plot-prop, fig.width = 10, fig.fullwidth = TRUE}
proportion_staff %>%
...
`The graph is too large. The data is squished into one tiny corner, not maximizing the data. It might also be helpful to have a bit of transparency for the overlap of the graph. I think the pie graphs just aren't helpful. It's hard to get a true idea of the size of each chunk in a pie chart. A bar graph might be better.`
...
fisheries %>%
  ...
  ggplot(aes(y = country, x = capture))+
  geom_col()+
  labs(
    title = "Country vs. Capture",
    y = "Country",
    x = "Capture"
  )
```

```{r fish-aqua, eval = TRUE}
fisheries %>%
  filter(aquaculture > 100000) %>%
  ggplot(aes(y = country, x = aquaculture))+
  geom_col()+
  labs(
    title = "Country vs. Capture",
    y = "Country",
